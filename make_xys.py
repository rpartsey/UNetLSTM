...
FOLDER = '/datasets/extra_space2/rpartsey/related_works/onera/train_labels'
...
for i_city in train_areas:
    path = os.path.join(FOLDER, f'{i_city}/cm/{i_city}-cm.tif')
...
df.to_csv('/datasets/extra_space2/rpartsey/related_works/onera/images_processed/myxys_train.csv', index=False, columns=["X", "Y", "image_ID", "transform_ID"])
...
df.to_csv('/datasets/extra_space2/rpartsey/related_works/onera/images_processed/myxys_val.csv', index=False, columns=["X", "Y", "image_ID", "transform_ID"])
